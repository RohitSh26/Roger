...
def test_parse_questions_drops_self_answering_items() -> None:
    raw = {
        "questions": [
            {
                "question": "What is build_charge_deps?",
                "options": {"A": "build charge deps", "B": "b", "C": "c", "D": "d"},
                "correct": "A",
            },
            {
    ...
    questions = router.parse_questions(
        raw, node_id="n1", difficulty="medium", tier=1, subject="build_charge_deps"
    )
    assert len(questions) == 1
...
def test_is_giveaway_rejects_meta_format_leakage() -> None:
    ...
    options = {"A": "billing.py", "B": "run_batch", "C": "_raw_charge_step", "D": "helpers"}
    leaked = (
        "Which of these must be updated to pass _is_gateway_400()? "
        "Correct: _charge_step(), because it calls it. Options: A: billing.py"
    )
    assert router._is_giveaway(leaked, options, "A", subject="_charge_step()")
    assert not router._is_giveaway(
        "Which of these must be updated when _is_gateway_400() changes?",
        options, "A", subject="_charge_step()",
    )


def test_normalize_options_trims_trailing_punctuation() -> None:
    raw = {"A": " invoice_record.py; ", "B": "b", "C": "c", "D": "d,"}
    options = router._normalize_options(raw)
    assert options is not None
    assert options["A"] == "invoice_record.py"
    assert options["D"] == "d"
...
def test_out_of_scope_allows_subject_and_visible_names() -> None:
    ...
    assert not router._is_out_of_scope(
        "What does _run_billing_job() return?", subject="FakeInvoiceClient", snippet=""
    )
...
def test_parse_questions_drops_out_of_scope_items() -> None:
    raw = {
        "questions": [
            {
                "question": "What is the immediate return value of _run_billing_job()?",
                "options": {"A": "FakeInvoiceClient", "B": "None", "C": "Error", "D": "Clean"},
                "correct": "B",
            },
            {
    ...
    questions = router.parse_questions(
        ...
        subject="FakeInvoiceClient", snippet=FAKE_CLIENT_SNIPPET,
    )
    assert len(questions) == 1
...
def test_parse_questions_rejects_lookup_trivia() -> None:
    raw = {
        "questions": [
            {
                "question": "Which file is normalize_term defined in?",
